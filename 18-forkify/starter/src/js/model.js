...
export const loadRecipe = async function (id) {
  try {
    const data = await APICall(`${API_URL}/${id}?key=${API_KEY}`);
...
export const loadSearchResults = async function (query) {
  try {
    ...
    const data = await APICall(`${API_URL}?search=${query}&key=${API_KEY}`);
...
// dev function to clear bookmarks
...
export const uploadRecipe = async function (newRecipe) {
  try {
    ...
    const data = await APICall(`${API_URL}?key=${API_KEY}`, recipe);
    state.recipe = createRecipeObject(data);
